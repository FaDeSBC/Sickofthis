...
export default function HomeScreen({ navigation }) {
  ...
  useEffect(() => {
    setLoading(true);
    const unsubscribe = subscribeToPost((fetchedPosts) => {
      const filteredPosts = fetchedPosts.filter(post => post.caseType === (isLost ? 'Lost' : 'Found'));
    ...
    <SafeAreaView style={styles.container}>
      <View style={styles.header}>
        ...
        <TouchableOpacity onPress={() => navigation.navigate('Search')}>
          <Image source={require('../../../app/assets/filterIcon.png')} style={styles.headerIcon} />
        </TouchableOpacity>
      </View>
      ...
      <View style={styles.toggleContainer}>
        ...
        >
          <Text style={[styles.toggleText, isLost && styles.toggleTextActive]}>Lost</Text>
        </TouchableOpacity>
        <TouchableOpacity
          style={[styles.toggleButton, !isLost && styles.toggleButtonActive]}
          onPress={() => setIsLost(false)}
        >
          <Text style={[styles.toggleText, !isLost && styles.toggleTextActive]}>Found</Text>
        </TouchableOpacity>
      </View>
      ...
      {loading ? (
        ...
        <View style={styles.emptyContainer}>
          <Text style={styles.emptyText}>No {isLost ? 'lost' : 'found'} items yet</Text>
        </View>
      ) : (
        <FlatList
          data={posts}
          ...
          refreshControl={
            <RefreshControl refreshing={refreshing} onRefresh={onRefresh} colors={['#50A296']} />
          }
        />
      )}
    </SafeAreaView>
